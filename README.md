...
5. 保持“纵向 G 自动计算”开启，让 `longitudinal_g` 随时速自动更新。
6. 在参数列表中勾选多个参数可叠加显示，选中的行是当前可编辑参数。
7. 使用 `Y Min / Y Max` 手动控制当前参数的显示范围；也可以直接拖动/缩放图表的 Y 轴，输入框会同步更新。多参数叠加时，各参数按自己的 Y 范围归一化显示。
8. 导出 CSV 给后期 UI 特效使用。
